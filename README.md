...
Streamlit을 활용한 격자 기반 위치 검색 웹 애플리케이션입니다. Google Places API의 한 번에 20개 결과 제한을 극복하기 위해 검색 범위를 격자로 나누어 더 많은 결과를 얻을 수 있도록 설계되었습니다. **향상된 이메일 추출 기능**과 **실시간 진행률 표시**로 최고의 사용자 경험을 제공합니다.
...
- **🆕 실시간 진행률 표시**: 
  - 격자 계산부터 이메일 추출까지 단계별 진행률 표시
  - 현재 처리 중인 격자와 키워드 실시간 확인
  - 예상 소요 시간 및 완료율 정보 제공
- **🆕 향상된 이메일 추출**: 
  - 표준 이메일 패턴 감지
...
검색 시작 후 실시간으로 진행 상황을 확인할 수 있습니다:

```
진행률: [████████████████████----------] 67%
상태: 검색 중... (5/8) 격자 3/4, 키워드: '카페'
```

**진행률 단계:**
- **2%**: 격자 포인트 계산
- **60%**: Places API 검색 (격자별 × 키워드별)
- **30%**: Places Details API (상세 정보 수집)
- **8%**: 이메일 추출 (웹사이트 크롤링)
...
```
extract/
├── app.py                 # Streamlit 메인 애플리케이션 (진행률 표시 포함)
├── backend/              
│   ├── __init__.py       
│   ├── config.py         # API 설정 관리
│   ├── grid_calculator.py # 격자 계산 로직
│   ├── places_api.py     # Google Places API 클라이언트
│   ├── search_engine.py  # 🆕 진행률 추적이 포함된 메인 검색 엔진
│   └── email_extractor.py # 🆕 향상된 이메일 추출 엔진
├── requirements.txt      # Python 의존성
├── demo.py              # 백엔드 로직 테스트용 데모 (진행률 시뮬레이션 포함)
└── README.md            # 프로젝트 문서
```
...
- **투명성**: 현재 어떤 작업이 진행 중인지 명확히 표시
- **예측 가능성**: 남은 시간과 완료율을 통한 대기 시간 예측
- **안심감**: 애플리케이션이 정상 작동 중임을 확인
- **디버깅**: 어느 단계에서 문제가 발생했는지 쉽게 파악
